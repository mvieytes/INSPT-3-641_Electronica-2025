...
error_web_server_t sent_callback(void* arg, struct tcp_pcb* tpcb, u16_t len) {
...
error_web_server_t http_recv(void* arg, struct tcp_pcb* tpcb, struct pbuf* p, err_t err) {
...
error_web_server_t init_http_server(void) {
    error_web_server_t error = TCP_OK;
