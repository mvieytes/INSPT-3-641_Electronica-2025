...
void OLED_scroll(bool on) {
    ...
    uint8_t cmds[] = {
        OLED_CMD_HORIZONTAL_SCROLL | 0x00,
        0x00,                                // dummy byte
        0x00,                                // start page 0
        0x00,                                // time interval
        ...
        0x00,                                // dummy byte
        0xFF,                                // dummy byte
        OLED_CMD_SCROLL | (on ? 0x01 : 0) // Start/stop scrolling
    };
...
#else
    uint8_t page_width = area->end_col - area->start_col + 1;
    uint8_t column = area->start_col + OLED_COLUMN_OFFSET;

    for (uint8_t page = area->start_page; page <= area->end_page; page++) {
        uint8_t cmds[] = {
            (uint8_t)(SH1106_CMD_PAGE_ADDR | page),
            (uint8_t)(SH1106_CMD_LOW_COL_ADDR | (column & 0x0F)),
            (uint8_t)(SH1106_CMD_HIGH_COL_ADDR | ((column >> 4) & 0x0F)),
        };

        OLED_send_cmd_list(cmds, count_of(cmds));
        OLED_send_buf(buf + ((page - area->start_page) * page_width), page_width);
    }
#endif
}

static void SetPixel(uint8_t* buf, int x, int y, bool on) {
    assert(x >= 0 && x < OLED_WIDTH && y >= 0 && y < OLED_HEIGHT);
