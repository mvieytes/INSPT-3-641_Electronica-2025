...
int main() {
    ...
    while (true) {
        ...
        if (rx_byte != 0) {
            ...
            printf("%c", rx_byte);
            ring_buffer_tx_uart_putc(rx_byte);
            ring_buffer_tx_uart_putc('\n');

            if (rx_byte == '1') {
                gpio_put(LED_PIN, 1);
                ring_buffer_tx_uart_puts("Led ON\n");
            } else if (rx_byte == '2') {
                gpio_put(LED_PIN, 0);
                ring_buffer_tx_uart_puts("Led OFF\n");
            } else {
                ring_buffer_tx_uart_puts("Comando incorrecto\n");
            }
            rx_byte = 0;
        }
